async function showDescriptionPanel() {
  document.querySelector('ytd-text-inline-expander[show-expand-button]')?.click();
  // wait 1 ms to allow the description panel to expand
  await new Promise(resolve => setTimeout(resolve, 1));
}

async function showTranscriptPanel() {
  await showDescriptionPanel(); 
  if (!document.querySelector('ytd-transcript-renderer')) {
    ...
    // Wait 1 second to allow the transcript panel to load
  ...
  if (request.action === 'extractTranscript' || request.action === 'extractTranscriptWithTitle') {
    (async () => {
      ...
      try {
        ...
        const header = `Transcript of a YouTube Video (url: https://www.youtube.com/watch?v=${videoId})
  ...
  `;
    ...
    (async () => {
      ...
      try {
        ...
        response.authorName = document.querySelector('[itemprop="author"] > [itemprop="name"]')?.getAttribute('content');
